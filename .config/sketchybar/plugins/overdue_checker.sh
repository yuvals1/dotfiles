...
TASKS_DIR="$HOME/tasks/1-⭐ in-progress"
OVERDUE_EMOJI="⏰"

# Check each file in the tasks directory
for file in "$TASKS_DIR"/*; do
    ...
    if [ -n "$due_line" ]; then
        ...
        if [ -n "$due_timestamp" ] && [ "$current_timestamp" -gt "$due_timestamp" ]; then
            ...
            mv "$file" "$TASKS_DIR/$new_filename"
        fi
    fi
done
...
overdue_count=$(find "$TASKS_DIR" -name "*$OVERDUE_EMOJI*" -type f | wc -l | tr -d ' ')
