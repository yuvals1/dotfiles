...
if [ $# -eq 0 ]; then
    echo "Usage: $0 {play-pause|next|previous|shuffle|repeat|radio_toggle|radio_1|radio_2|radio_3|seek-forward|seek-backward|add-to-playlist}"
    echo ""
    echo "Commands:"
...
# Validate command
case "$COMMAND" in
    "play-pause"|"next"|"previous"|"shuffle"|"repeat"|"radio_toggle"|"radio_1"|"radio_2"|"radio_3"|"seek-forward"|"seek-backward"|"add-to-playlist")
        # Valid command
        ;;
    *)
        echo "Error: Invalid command '$COMMAND'"
        echo "Valid commands: play-pause, next, previous, shuffle, repeat, radio_toggle, radio_1, radio_2, radio_3, seek-forward, seek-backward, add-to-playlist"
        exit 1
        ;;
esac
